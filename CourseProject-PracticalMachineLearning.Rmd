...
We first quickly load data from the URLs provided. I check the data and found out that data has three types of empty data. NA, blank and #DIC/0!. So while reading data into our variables, we cosnider these three types of data as NA. Later ontTHis will help us to clean up further.
...
training_60pc <- training[train_index, ]; testing_40pc <- training[-train_index, ]
dim(training_60pc); dim(testing_40pc)
```
...
Now we a training dataset that does not have near zero variance variables in it. Now we remove the first column od dataset which is an id variable. We do not what this column data to interfere with our Model.
``` {r}
...
``` {r}
training_60pc_Temp <- training_60pc #creating another subset to iterate in loop
for(i in 1:length(training_60pc)) { #for every column in the training dataset
        if( sum( is.na( training_60pc[, i] ) ) /nrow(training_60pc) >= .6 ) { #if n?? NAs > 60% of total observations
        for(j in 1:length(training_60pc_Temp)) {
            if( length( grep(names(training_60pc[i]), names(training_60pc_Temp)[j]) ) ==1)  { #if the columns are the same:
                training_60pc_Temp <- training_60pc_Temp[ , -j] #Remove that column
            }   
        } 
    }
}
...
``` {r}
clean1 <- colnames(training_60pc)
clean2 <- colnames(training_60pc[, -58]) #already with classe column removed
testing_40pc <- testing_40pc[clean1]


#To check the new N?? of observations
dim(testing_40pc)

#Do the same thing to our orignial test dataset.
...
testing <- rbind(training_60pc[2, -58] , testing) #note row 2 does not mean anything, this will be removed right.. now:
testing <- testing[-1,]
